...
3. 输入番剧名称，点击“搜索来源”。确认每个网站选中的是同一部、同一季；唯一规范化同名匹配会自动预选，明确的“第二季 / 第2季 / Season 2”等季号可对应。
4. 选择至少两个网站，默认使用“快速比较”，点击“开始画质检测”。集数留空时自动选择覆盖网站最多的最早一集，也可指定集号。分数接近时可改用“完整比较”复测。
...
- 快速模式比较三个正文位置；完整模式尝试四个位置。每个位置仍检查三个相邻画面，所有入榜来源必须拥有至少三个共同位置。快速模式的取样覆盖少于完整模式，两次不同取样的分数不宜直接比较。
- 分辨率、编码和可用码率用于展示、安排参考源顺序，不会用于淘汰。低分辨率高质量来源仍有取样和获胜机会；码率缺失显示“未提供”，不同编码间不按码率硬排。
...
- 网站响应有大小和超时限制；正文含压缩流受总截止时间保护。异常慢速响应头仍不保证硬实时截止；已发出的搜索与章节请求、首次模型加载可能需要等待才响应取消。
...
本次本地 HTTP 基准中，三帧提取中位数从0.540秒降至0.196秒，评分图像素一致；粗匹配PNG体积降低约95%。两次实际两站四线路快速检测耗时89.0秒与87.6秒，不能据此保证其他网站的速度。模型短基准中三帧一批、8个线程均有收益，但真实任务仍受网络、解码及其他程序负载影响。
